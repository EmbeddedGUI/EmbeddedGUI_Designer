...
def _build_and_materialize_smoke_project(
    ...
    """Build the smoke project and materialize its generated outputs via the shared saved scaffold helper."""
    widgets, page_customizer, meta = _build_smoke_project_inputs()
...
def run_smoke(sdk_root: str = "", work_dir: str = "", keep_temp: bool = False) -> int:
    ...
    if work_dir:
        temp_root = create_temp_workspace(Path(work_dir).resolve(), "ui_designer_preview_smoke_")
    else:
        temp_root = create_repo_temp_workspace(REPO_ROOT, "ui_designer_preview_smoke_", "preview_smoke")
