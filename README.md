...
Use `Build -> Release History...` to inspect, time-filter, refresh, copy, or export prior release summaries as text or JSON, preview manifests or build logs directly inside the Designer, and reopen with the last-used filters still applied.
Use `Build -> Repository Health...` to inspect SDK submodule state, release smoke sample presence, stale workspace temp directories, filter to critical issues, switch between text or JSON reports, export the current report view, and reopen with the previous view settings intact.
