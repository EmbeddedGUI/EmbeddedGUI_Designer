...
Use `Build -> Repository Health...` to inspect SDK submodule state, release smoke sample presence, stale workspace temp directories, filter to critical issues or blocked stale directories, reset back to the default view, switch between text or JSON reports with embedded summary/count metadata, copy short summaries or JSON reports for issue reports, choose and open the currently filtered stale temp directory directly, export the current report view, and reopen with the previous view settings intact.
