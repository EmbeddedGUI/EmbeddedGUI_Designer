...
_TEXT_ONLY_ICON_CACHE: dict[int, QIcon] = {}


def _text_only_icon(size=16):
    icon_size = max(int(size or 16), 1)
    cached = _TEXT_ONLY_ICON_CACHE.get(icon_size)
    if cached is not None:
        return cached
    pixmap = QPixmap(icon_size, icon_size)
    pixmap.fill(Qt.transparent)
    icon = QIcon(pixmap)
    _TEXT_ONLY_ICON_CACHE[icon_size] = icon
    return icon
...
class StatusCenterPanel(QWidget):
    ...
    def _init_ui(self):
        ...
        metrics_layout.setHorizontalSpacing(_SPACE_XS)
        metrics_layout.setVerticalSpacing(_SPACE_XS)
        ...
        health_layout.setContentsMargins(_SPACE_XS, _SPACE_XS, _SPACE_XS, _SPACE_XS)
        health_layout.setSpacing(4)

        health_title_row = QHBoxLayout()
        health_title_row.setContentsMargins(0, 0, 0, 0)
        health_title_row.setSpacing(_SPACE_XS)
        ...
        runtime_layout.setContentsMargins(_SPACE_XS, _SPACE_XS, _SPACE_XS, _SPACE_XS)
        runtime_layout.setSpacing(4)
        runtime_title_row = QHBoxLayout()
        runtime_title_row.setContentsMargins(0, 0, 0, 0)
        runtime_title_row.setSpacing(_SPACE_XS)
    ...
    def _create_metric(self, grid_layout, row, col, label, value, action_key=""):
        ...
        card_layout.setContentsMargins(_SPACE_XS, _SPACE_XS, _SPACE_XS, _SPACE_XS)
        card_layout.setSpacing(_SPACE_XS)
        top = QHBoxLayout()
    ...
    def _set_toolbutton_popup_mode(self, button, mode):
        value = int(mode)
        if int(button.property("_status_center_popup_mode_snapshot") or -1) == value:
            return
    ...
    def _set_widget_icon(self, widget, icon_key, size=16):
        resolved_key = str(icon_key or "").strip()
        resolved_size = max(int(size or 16), 0)
        current_key = str(widget.property("_status_center_icon_snapshot") or "")
        current_size = int(widget.property("_status_center_icon_size_snapshot") or -1)
        if current_key == resolved_key and current_size == resolved_size:
            return
        widget.setIcon(_text_only_icon(resolved_size) if resolved_key else QIcon())
        if hasattr(widget, "setIconSize"):
            try:
                widget.setIconSize(QSize(0, 0))
            except Exception:
                pass
        widget.setProperty("iconKey", resolved_key or None)
        widget.setProperty("_status_center_icon_snapshot", resolved_key)
        widget.setProperty("_status_center_icon_size_snapshot", resolved_size)
    ...
    def _build_action_button(self, text, icon_key, action_key):
        ...
        self._set_widget_icon(button, icon_key, size=16)
    ...
    def _refresh_repeat_action_menu(self):
        ...
        for action_key in self._recent_actions:
            ...
            menu_action.setIcon(_text_only_icon(16))
            menu_action.setProperty("iconKey", self._action_icon_key(action_key) or None)
        ...
        clear_action.setIcon(_text_only_icon(16))
        clear_action.setProperty("iconKey", "history")
    ...
    def set_status(
        ...
        *,
        ...
        self._set_widget_text(self._diag_btn, self._diagnostics_button_text(diag_total))
        self._set_widget_text(self._history_btn, self._history_button_text(dirty_count))
        self._set_widget_text(self._structure_btn, self._structure_button_text(selection_total))
        self._set_widget_text(self._debug_btn, self._debug_button_text(can_compile, runtime_text))
        self._set_widget_text(self._project_btn, self._project_button_text(sdk_ready))
        ...
        self._set_accessible_name(
            self._suggested_action_summary_label,
        ...
        self._set_accessible_name(self._error_value, f"Errors value: {self._error_value.text()}")
        self._set_accessible_name(self._warning_value, f"Warnings value: {self._warning_value.text()}")
        self._set_accessible_name(self._info_value, f"Info value: {self._info_value.text()}")
        ...
        self._set_accessible_name(self._error_bar, error_share)
        self._set_accessible_name(self._warning_bar, warning_share)
        self._set_accessible_name(self._info_bar, info_share)
        ...
        self._set_accessible_name(self._health_title, self._diagnostic_title_accessible_name(diag_total))
        ...
        if runtime_text:
            self._set_widget_text(self._runtime_title, self._runtime_title_text(runtime_text))
            self._set_hint(self._runtime_title, self._runtime_title_tooltip(runtime_text))
            self._set_accessible_name(self._runtime_title, self._runtime_title_accessible_name(runtime_text))
            self._set_widget_visible(self._runtime_label, True)
            self._set_widget_text(self._runtime_label, runtime_text)
            ...
            self._set_widget_text(self._runtime_title, self._runtime_title_text(""))
            self._set_hint(self._runtime_title, self._runtime_title_tooltip(""))
            self._set_accessible_name(self._runtime_title, self._runtime_title_accessible_name(""))
            self._set_widget_visible(self._runtime_label, False)
            self._set_widget_text(self._runtime_label, "No runtime errors.")
            self._set_hint(self._runtime_label, "No runtime errors.")
            self._set_accessible_name(self._runtime_label, "Runtime details: No runtime errors.")
            self._set_accessible_name(self._runtime_panel, self._runtime_panel_accessible_name(""))
            ...
            self._set_accessible_name(self._runtime_chip, self._runtime_chip_accessible_name(""))
            self._set_widget_visible(self._runtime_chip, False)
