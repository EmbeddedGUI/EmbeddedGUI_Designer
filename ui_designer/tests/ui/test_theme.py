...
def test_scrollbars_follow_theme_surface_tokens():
    for mode in ("light", "dark"):
        ...
        assert f"min-width: {t['h_tab_min']}px;" in horizontal_handle
...
def test_icon_scale_matches_ide_redesign_spec():
    for mode in ("light", "dark"):
        t = theme_tokens(mode)
        css = _build_stylesheet(mode)
        favorite_button = css.split("#widget_browser_favorite_button {", 1)[1].split("}", 1)[0]

        assert int(t["icon_xs"]) == 14
        assert int(t["icon_sm"]) == 16
        assert int(t["icon_md"]) == 18
        assert int(t["icon_lg"]) == 20
...
def test_animations_panel_styles_use_engineering_surface_tokens():
    for mode in ("light", "dark"):
        ...
        assert f"background-color: {t['panel_alt']};" in table
        assert "border-radius: 0px;" in table
        assert f"padding: {t['pad_btn_v']}px {t['pad_input_h']}px;" in table_header
...
def test_property_panel_spin_arrow_qss_tracks_compact_icon_size():
    ...
    assert (
        f"padding: 0px {theme_module._PROPERTY_PANEL_SPIN_FIELD_RIGHT_PADDING}px "
        f"0px {theme_module._PROPERTY_PANEL_SPIN_FIELD_LEFT_PADDING}px;"
    ) in spin_field
