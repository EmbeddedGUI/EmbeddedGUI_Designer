...
class TestStatusCenterPanel:
    ...
    def test_first_warning_button_accessible_name_skips_no_op_rewrites(self, qapp, monkeypatch):
        from ui_designer.ui.status_center_panel import StatusCenterPanel

        panel = StatusCenterPanel()
        panel._first_warning_btn.setProperty("_status_center_accessible_snapshot", None)

        accessible_calls = 0
        original_set_accessible_name = panel._first_warning_btn.setAccessibleName
        ...
        monkeypatch.setattr(panel._first_warning_btn, "setAccessibleName", counted_set_accessible_name)
    ...
    def test_debug_button_text_skips_no_op_rewrites(self, qapp, monkeypatch):
        from ui_designer.ui.status_center_panel import StatusCenterPanel

        panel = StatusCenterPanel()
        panel._debug_btn.setProperty("_status_center_text_snapshot", None)

        text_calls = 0
        original_set_text = panel._debug_btn.setText
        ...
        monkeypatch.setattr(panel._debug_btn, "setText", counted_set_text)
        ...
        panel.set_status(sdk_ready=True, can_compile=False, dirty_pages=1)
        assert text_calls == 2

        panel.deleteLater()

    def test_debug_button_accessible_name_skips_no_op_rewrites(self, qapp, monkeypatch):
        from ui_designer.ui.status_center_panel import StatusCenterPanel

        panel = StatusCenterPanel()
        panel._debug_btn.setProperty("_status_center_accessible_snapshot", None)

        accessible_calls = 0
        original_set_accessible_name = panel._debug_btn.setAccessibleName
        ...
        monkeypatch.setattr(panel._debug_btn, "setAccessibleName", counted_set_accessible_name)
    ...
    def test_suggested_action_button_text_skips_no_op_rewrites(self, qapp, monkeypatch):
        from ui_designer.ui.status_center_panel import StatusCenterPanel

        panel = StatusCenterPanel()
        panel._suggested_action_button.setProperty("_status_center_text_snapshot", None)

        text_calls = 0
        original_set_text = panel._suggested_action_button.setText
        ...
        monkeypatch.setattr(panel._suggested_action_button, "setText", counted_set_text)
    ...
    def test_suggested_action_button_accessible_name_skips_no_op_rewrites(self, qapp, monkeypatch):
        from ui_designer.ui.status_center_panel import StatusCenterPanel

        panel = StatusCenterPanel()
        panel._suggested_action_button.setProperty("_status_center_accessible_snapshot", None)

        accessible_calls = 0
        original_set_accessible_name = panel._suggested_action_button.setAccessibleName
        ...
        monkeypatch.setattr(panel._suggested_action_button, "setAccessibleName", counted_set_accessible_name)
    ...
    def test_header_subtitle_text_skips_no_op_rewrites(self, qapp, monkeypatch):
        from ui_designer.ui.status_center_panel import StatusCenterPanel

        panel = StatusCenterPanel()
        panel._header_subtitle.setProperty("_status_center_text_snapshot", None)

        text_calls = 0
        original_set_text = panel._header_subtitle.setText
        ...
        monkeypatch.setattr(panel._header_subtitle, "setText", counted_set_text)
    ...
    def test_header_subtitle_accessible_name_skips_no_op_rewrites(self, qapp, monkeypatch):
        from ui_designer.ui.status_center_panel import StatusCenterPanel

        panel = StatusCenterPanel()
        panel._header_subtitle.setProperty("_status_center_accessible_snapshot", None)

        accessible_calls = 0
        original_set_accessible_name = panel._header_subtitle.setAccessibleName
        ...
        monkeypatch.setattr(panel._header_subtitle, "setAccessibleName", counted_set_accessible_name)
    ...
    def test_warning_value_accessible_name_skips_no_op_rewrites(self, qapp, monkeypatch):
        from ui_designer.ui.status_center_panel import StatusCenterPanel

        panel = StatusCenterPanel()
        panel._warning_value.setProperty("_status_center_accessible_snapshot", None)

        accessible_calls = 0
        original_set_accessible_name = panel._warning_value.setAccessibleName
        ...
        monkeypatch.setattr(panel._warning_value, "setAccessibleName", counted_set_accessible_name)
        ...
        panel.set_status(diagnostics_errors=2, diagnostics_warnings=2, diagnostics_infos=1, dirty_pages=1)
        assert accessible_calls == 2

        panel.deleteLater()

    def test_warning_bar_accessible_name_skips_no_op_rewrites(self, qapp, monkeypatch):
        from ui_designer.ui.status_center_panel import StatusCenterPanel

        panel = StatusCenterPanel()
        panel._warning_bar.setProperty("_status_center_accessible_snapshot", None)

        accessible_calls = 0
        original_set_accessible_name = panel._warning_bar.setAccessibleName
        ...
        monkeypatch.setattr(panel._warning_bar, "setAccessibleName", counted_set_accessible_name)
    ...
    def test_sdk_card_accessible_name_skips_no_op_rewrites(self, qapp, monkeypatch):
        from ui_designer.ui.status_center_panel import StatusCenterPanel

        panel = StatusCenterPanel()
        panel._sdk_card.setProperty("_status_center_accessible_snapshot", None)

        accessible_calls = 0
        original_set_accessible_name = panel._sdk_card.setAccessibleName
        ...
        monkeypatch.setattr(panel._sdk_card, "setAccessibleName", counted_set_accessible_name)

        panel.set_status(sdk_ready=True)
        assert accessible_calls == 1

        panel.set_status(sdk_ready=True, dirty_pages=1)
        assert accessible_calls == 1

        panel.set_status(sdk_ready=False, dirty_pages=1)
        assert accessible_calls == 2
