...
class TestWorkspaceHelpers:
    ...
    def test_sdk_example_path_helpers(self, tmp_path):
        sdk_root = tmp_path / "sdk"
        ...
        assert sdk_example_project_path(str(sdk_root), "HelloApp") == normalize_path(
            str(sdk_root / "example" / "HelloApp" / "HelloApp.egui")
        )
    ...
    def test_resolve_designer_sdk_root_prefers_cli_root(self, tmp_path, monkeypatch):
        repo_root = tmp_path / "EmbeddedGUI_Designer"
        sdk_root = tmp_path / "CustomSdk"
        repo_root.mkdir()
        _create_sdk_root(sdk_root)
        monkeypatch.setenv(SDK_ROOT_ENV_VAR, str(tmp_path / "ignored-sdk"))

        assert resolve_designer_sdk_root(
            repo_root=str(repo_root),
            cli_sdk_root=str(sdk_root),
        ) == normalize_path(str(sdk_root))

    def test_resolve_designer_sdk_root_rejects_invalid_cli_root(self, tmp_path):
        repo_root = tmp_path / "EmbeddedGUI_Designer"
        repo_root.mkdir()
        ...
        with pytest.raises(ValueError, match="invalid EmbeddedGUI SDK root"):
            resolve_designer_sdk_root(
                repo_root=str(repo_root),
                cli_sdk_root=str(tmp_path / "missing-sdk"),
                env={},
            )

    def test_resolve_designer_sdk_root_rejects_invalid_env_root(self, tmp_path, monkeypatch):
        repo_root = tmp_path / "EmbeddedGUI_Designer"
        repo_root.mkdir()
        monkeypatch.setenv(SDK_ROOT_ENV_VAR, str(tmp_path / "missing-sdk"))

        with pytest.raises(ValueError, match=SDK_ROOT_ENV_VAR):
            resolve_designer_sdk_root(
                repo_root=str(repo_root),
            )
    ...
    def test_require_designer_sdk_root_reports_help_when_missing(self, tmp_path):
        ...
        with pytest.raises(RuntimeError, match="EmbeddedGUI SDK root not found") as exc_info:
            require_designer_sdk_root(repo_root=str(repo_root), env={})
    ...
    def test_describe_designer_sdk_root_help_mentions_flag_env_submodule_and_sibling(self, tmp_path):
        ...
        text = describe_designer_sdk_root_help(str(repo_root))
